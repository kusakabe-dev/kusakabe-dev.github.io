...
1. **Apple Developer Program の登録状況**: すでに登録済みでしょうか？未登録の場合、Phase 6 の開始が遅れる可能性があります。
2. **AdMob iOS版のアプリID**: Android版と同じAdMobアカウントでiOSアプリを追加しますか？
3. **CocoaPods vs SPM**: iOS依存ライブラリの管理方法について希望はありますか？（KMPプロジェクトでは CocoaPods が一般的ですが、SPM も選択肢です）
4. **TestFlight テスター**: 社内・知人テスターの有無によりPhase 6の進め方が変わります。
5. **iOS版リリースの目標時期**: ロードマップでは10週間のスパンが見込まれていますが、具体的な目標日はありますか？
